...
class LancamentoProcessor {
  ...
  static processarAdiantamentoFornecedores(row: ExcelRow): string[] {
    ...
    const importLocation = Number(row[ColunaExcel.IMPORT_LOCATION]);
    if (
      ![TipoEmpresa.MATRIZ, TipoEmpresa.FILIAL2, TipoEmpresa.FILIAL3].includes(
        importLocation,
      )
    )
      return results;
    ...
    if (contaBancaria === '13') {
      ...
      if (importLocation === TipoEmpresa.MATRIZ) {
        ...
        // filial
        results.push(
          `${localImportacao};${dataFormatada};579;1516;${valorS};${historicoBase}`,
        );
        // extra na matriz
        const extraAcc = getExtraAccount(importLocation);
  ...
  static processarDespesasDiversas(row: ExcelRow): string[] {
    ...
    if (jurosTotal !== '0.00') {
      results.push(
        `${local};${data};1381;1516;${jurosTotal};0;Pagto Juros s/ Duplic. N: ${iText} - ${gText}`,
      );
      results.push(
    ...
    if (vAE !== '0.00') {
      results.push(
        `${local};${data};1383;1516;${vAE};0;Pagto Multa s/ Duplic. N: ${iText} - ${gText}`,
      );
      results.push(
