 ...
 */
enum TipoEmpresa {
  MATRIZ = 13,    // Matriz - LONDRINA
  FILIAL2 = 14,   // Filial 2 - PRUDENTE
  FILIAL3 = 15    // Filial 3 - UMUARAMA
}
 ...
 * @const {Object}
 */
const NP_IMPORT_MAP: { [key: number]: string } = {
 ...
 * @const {Map}
 */
const NP_EXTRA_ADIANT_MAP = new Map<number, string>([
  [TipoEmpresa.FILIAL2, '1513'],  // Código para filial 2
  [TipoEmpresa.FILIAL3, '5104'],  // Código para filial 3
]);

const EXTRA_ACCOUNT_MAP = new Map<number, string>([
  [TipoEmpresa.FILIAL2, '1513'],
  [TipoEmpresa.FILIAL3, '5104'],
]);
 ...
 * @const {Map}
 */
const NP_EXTRA_DISCOUNT_MAP = new Map<number, string>([
  [TipoEmpresa.FILIAL2, '1513'],  // Código para filial 2
  [TipoEmpresa.FILIAL3, '5104'],  // Código para filial 3
]);
 ...
 * @const {Map}
 */
const NP_EXTRA_JUROS_MULTA_MAP = new Map<number, string>([
  [TipoEmpresa.FILIAL2, '1513'],  // Código para filial 2
  [TipoEmpresa.FILIAL3, '5104'],  // Código para filial 3
]);
...
class LancamentoProcessor {
  ...
  static processarExtraLancamentoMatrizDuplicata(row: ExcelRow): string {
    ...
    const localImportacao = importLocation === TipoEmpresa.FILIAL3 ? "0001" : NP_IMPORT_MAP[importLocation];
  ...
  static processarAdiantamentoFornecedores(row: ExcelRow): string[] {
    ...
    const importLocation = Number(row[ColunaExcel.IMPORT_LOCATION]);
    if (![TipoEmpresa.MATRIZ, TipoEmpresa.FILIAL2, TipoEmpresa.FILIAL3]
        .includes(importLocation)) return results;
  
    const localImportacao = NP_IMPORT_MAP[importLocation];
    ...
    if (contaBancaria === '13') {
      ...
      if (importLocation === TipoEmpresa.MATRIZ) {
        ...
        // filial
        results.push(`${localImportacao};${dataFormatada};579;1516;${valorS};${historicoBase}`);
        // extra na matriz
        const extraAcc = getExtraAccount(importLocation);
  ...
  static processarDespesasDiversas(row: ExcelRow): string[] {
    ...
    // FILIAL: crédito = 1516
    results.push(`${local};${data};${contaDeb};1516;${vO};${historico}`);
    ...
    if (jurosTotal !== '0.00') {
      results.push(`${local};${data};1381;1516;${jurosTotal};0;Pagto Juros s/ Duplic. N: ${iText} - ${gText}`);
      results.push(`0001;${data};${extraAcc};${codV};${jurosTotal};0;Pagto Juros s/ Duplic. N: ${iText} - ${gText}`);
    }
    if (vAE   !== '0.00') {
      results.push(`${local};${data};1383;1516;${vAE};0;Pagto Multa s/ Duplic. N: ${iText} - ${gText}`);
...
class EnergiaDespesasProcessor {
  ...
  static processarEnergia(row: ExcelRow): string[] {
    ...
    if (codV === "13") {
      ...
      } else if (importLocation === TipoEmpresa.FILIAL2 || importLocation === TipoEmpresa.FILIAL3) {
  ...
  static processarDespesasBancarias(row: ExcelRow): string[] {
    const results: string[] = [];
    const importLocation = Number(row[ColunaExcel.IMPORT_LOCATION]);
    if (![TipoEmpresa.MATRIZ, TipoEmpresa.FILIAL2, TipoEmpresa.FILIAL3].includes(importLocation)) return results;
    const localImportacao = NP_IMPORT_MAP[importLocation];
...
class FiscalProcessor {
  ...
  static async processarFiscal326(
    ...
    for (const row of rows) {
      ...
      const importLocation = Number(row[ColunaExcel.IMPORT_LOCATION]);
      if (![TipoEmpresa.MATRIZ, TipoEmpresa.FILIAL2, TipoEmpresa.FILIAL3].includes(importLocation)) continue;
      ...
      if (!chaveDuplicata) {
        duplicatasNaoEncontradas.push({
          'Código': NP_IMPORT_MAP[importLocation] || '',
      ...
      const tipoEmpresaFiscal = 
        ...
        importLocation === TipoEmpresa.FILIAL2 ? '2' : 
        importLocation === TipoEmpresa.FILIAL3 ? '3' : '1';
...
class Processador326 {
  ...
  static async processarArquivo326(
    ...
    try {
      console.log('Iniciando processamento da Regra 326 (N&P)...');
  ...
  private static processarContabil326(rows: ExcelRow[]): string[] {
    ...
    for (const row of rows) {
      ...
      if (tipoOperacao === TipoOperacao.ADIANTAMENTO_FORNECEDORES) {
        ...
        const isFilial = [TipoEmpresa.FILIAL2].includes(importLocation);
