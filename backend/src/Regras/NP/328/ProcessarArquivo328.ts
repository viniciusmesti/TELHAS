...
export function processarRegra328(rows: any[]): string[] {
  ...
  rows.forEach((row, index) => {
    ...
    let local = '';
    if (empresa === '13')
      local = '0001'; // Matriz - LONDRINA
    else if (empresa === '14')
      local = '0002'; // Filial 2 – PRUDENTE
    ...
    console.log(
      `Linha ${index + 1} - Código=${codigoRelatorio}, Empresa=${empresa}, Banco=${banco}, Data=${dataBaixa}, Valor=${valorBaixa}`,
    );
    ...
    if (banco === 13) {
      ...
      output.push(
        `${local};${dataBaixa};893;13;${valorBaixa};${historicoBase}`,
      );
    } else if (banco) {
      ...
      if (empresa === '13') {
        ...
        output.push(
          `${local};${dataBaixa};893;${banco};${valorBaixa};${historicoBase}`,
        );
      } else if (empresa === '14' || empresa === '15') {
        // Se for Filial (empresa "14" ou "15")
        // Lançamento na própria filial:
        output.push(
          `${local};${dataBaixa};893;1516;${valorBaixa};${historicoBase}`,
        );
        ...
        output.push(
          `0001;${dataBaixa};${extraAccount};${banco};${valorBaixa};${historicoBase}`,
        );
      }
    }
  });
...
export async function processarArquivo328(
