...
export function processarRegra329(rows: any[]): string[] {
  ...
  rows.forEach((row, index) => {
    ...
    if (grupo1.includes(filialStr)) {
      ...
      console.log(`Linha ${index + 1} ignorada - Filial inválida: ${filialStr}`);
      return;
    }
    ...
    if (grupo === 1) {
      ...
      if (filialStr === "13") {
        ...
        output.push(`${local};${dataBaixa};1483;${banco};${valorBaixa};1850;${historico}`);
      } else if (filialStr === "14" || filialStr === "15") {
        // Nas filiais:
        // Lançamento na filial: D = 1483, C = 1516 (conta corrente fixa)
        output.push(`${local};${dataBaixa};1483;1516;${valorBaixa};1850;${historico}`);
        // Lançamento extra na Matriz:
        let extraAccount = "";
        if (filialStr === "14") extraAccount = "1513";
        else if (filialStr === "15") extraAccount = "5104";
        output.push(`0001;${dataBaixa};${extraAccount};${banco};${valorBaixa};1850;${historico}`);
      }
    } else if (grupo === 2) {
      // 2º SITUAÇÃO (coluna B = 513, 514 ou 515)
      if (filialStr === "513") {
        // Matriz: D = 863, C = BANCO
        output.push(`${local};${dataBaixa};863;${banco};${valorBaixa};1850;${historico}`);
      } else if (filialStr === "514" || filialStr === "515") {
        // Nas filiais:
        // Lançamento na filial: D = 863, C = 1516 (fixo)
        output.push(`${local};${dataBaixa};863;1516;${valorBaixa};1850;${historico}`);
        // Lançamento extra na Matriz:
        let extraAccount = "";
