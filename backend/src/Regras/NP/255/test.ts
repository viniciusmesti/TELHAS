...
async function testarProcessamento() {
  try {
    console.log('Iniciando o teste da Regra 255 para a empresa N&P...');
    
    // Ajuste os caminhos conforme sua estrutura de pastas:
    const inputExcelPath = path.join(__dirname, '..', '..', '..', 'uploads', 'N&P', 'uploads', '255.xlsx');
    const outputTxtPath = path.join(__dirname, '..', '..', '..', 'uploads', 'N&P', 'saida', 'saida255.txt');
