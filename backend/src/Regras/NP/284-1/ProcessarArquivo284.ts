...
export function transformData(rows: any[]): string[] {
  ...
  rows.forEach((row, index) => {
    ...
    const historicoBase =
      `${normalizeText(maquina)} - ${historicoG} - ${historicoI}`.trim();
    // Coluna V: Banco
    ...
    if (filial === '13') {
      ...
      output.push(
        `${local};${dataBaixa};1325;${contaMaquina};${taxaAdministrativa};1360;${historicoBase}`,
      );
    } else if (filial === '14' || filial === '15') {
      // Filial
      output.push(
        `${local};${dataBaixa};1514;${contaMaquina};${valorBaixa};1189;${historicoBase}`,
      );
      output.push(
        `${local};${dataBaixa};1325;${contaMaquina};${taxaAdministrativa};1360;${historicoBase}`,
      );
  ...
  console.log(
    `Transformação completa. Total de linhas processadas: ${output.length}`,
  );
  return output;
}
