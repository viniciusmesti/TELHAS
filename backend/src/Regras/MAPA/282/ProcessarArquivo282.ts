...
function processRow(row: any[], index: number, output: string[]): void {
  ...
  if (empresa === '6') {
    ...
    add(
      ...
      cnpj,
      valorDesdobramento,
      buildHistorico('1188', histDoc, histCliente),
    );
    add(
      MATRIZ_CODE,
      banco,
      ...
      juros,
      buildHistorico('1202', histDoc, histCliente),
    );
    add(
      MATRIZ_CODE,
      CONTA_DESCONTO,
      banco,
      desconto,
      buildHistorico('2082', histDoc, histCliente),
    );
    add(
      MATRIZ_CODE,
      banco,
      CONTA_MULTA,
      multa,
      buildHistorico('1997', histDoc, histCliente),
    );
  }
