...
const CONFIG = {
  COLUNAS: {
    ...
    IMPORT_LOCATION: 2,      // Coluna B: deve conter 6 ou 9
  ...
  VALORES: {
    ...
    TIPO_EMPRESA_MATRIZ: 6,
    TIPO_EMPRESA_FILIAL: 9,
  },
  MENSAGENS: {
    ERRO_PROCESSAMENTO: '❌ Erro no processamento da regra MAPA 289:',
...
const MAPEAMENTOS = {
  ...
  IMPORTACAO: {
    ...
    [CONFIG.VALORES.TIPO_EMPRESA_FILIAL]: '0002',  // Filial 2 – PRUDENTE
  },
  EXTRA: {
    [CONFIG.VALORES.TIPO_EMPRESA_FILIAL]: '1513',  // Para Filial 2 – PRUDENTE
  }
};
...
// ============================================================================
export async function processarArquivo289Mapa(
  inputExcelPath: string,
  duplicatasExcelPath: string
): Promise<ResultadoProcessamento> {
  try {
    console.log('📂 Carregando arquivos MAPA 289...');
...
function validarDadosPagamento(dados: DadosPagamento): boolean {
  return dados.relatorio === CONFIG.VALORES.RELATORIO_ESPERADO &&
         [CONFIG.VALORES.TIPO_EMPRESA_MATRIZ, CONFIG.VALORES.TIPO_EMPRESA_FILIAL].includes(dados.importLocation);
}
...
function processarFiscal(
  ...
  const tipoEmpresaFiscal = dados.importLocation === CONFIG.VALORES.TIPO_EMPRESA_MATRIZ ? '1' : '2';
  ...
  if (dados.meioPagamento !== CONFIG.VALORES.MEIO_PAGAMENTO_CAIXA && dados.importLocation !== CONFIG.VALORES.TIPO_EMPRESA_MATRIZ) {
