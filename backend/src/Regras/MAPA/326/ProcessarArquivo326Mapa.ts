...
class LancamentoProcessor {
  ...
  static processarExtraLancamentoMatrizDuplicata(row: ExcelRow): string {
    ...
    const localImportacao =
      importLocation === TipoEmpresa.FILIAL
        ? '0001'
        : MAPA_IMPORT_MAP[importLocation];
  ...
  static processarAdiantamentoFornecedores(row: ExcelRow): string[] {
    ...
    const importLocation = Number(row[ColunaExcel.IMPORT_LOCATION]);
    if (![TipoEmpresa.MATRIZ, TipoEmpresa.FILIAL].includes(importLocation))
      return results;
    ...
    const valorS = row[ColunaExcel.VALOR_S]
      ? parseFloat(row[ColunaExcel.VALOR_S]).toFixed(2)
      : '0.00';
    ...
    if (contaBancaria === '13') {
      results.push(
        `${localImportacao};${dataFormatada};579;13;${valorS};${historico}`,
      );
    } else {
      if (importLocation === TipoEmpresa.MATRIZ) {
        results.push(
          `${localImportacao};${dataFormatada};579;${contaBancaria};${valorS};${historico}`,
        );
      } else if (importLocation === TipoEmpresa.FILIAL) {
        results.push(
          `${localImportacao};${dataFormatada};579;1516;${valorS};${historico}`,
        );
        results.push(
          `0001;${dataFormatada};${MAPA_EXTRA_ADIANT_MAP.get(importLocation)};${contaBancaria};${valorS};${historico}`,
        );
      }
    }
    return results;
  ...
  static processarDespesasDiversas(row: ExcelRow): string[] {
    ...
    const valorO = row[ColunaExcel.VALOR_O]
      ? parseFloat(row[ColunaExcel.VALOR_O]).toFixed(2)
      : '0.00';
    const valorAC = row[ColunaExcel.VALOR_AC]
      ? parseFloat(row[ColunaExcel.VALOR_AC]).toFixed(2)
      : '0.00';
    const valorAD = row[ColunaExcel.VALOR_AD]
      ? parseFloat(row[ColunaExcel.VALOR_AD]).toFixed(2)
      : '0.00';
    const valorAE = row[ColunaExcel.VALOR_AE]
      ? parseFloat(row[ColunaExcel.VALOR_AE]).toFixed(2)
      : '0.00';
    const valorAF = row[ColunaExcel.VALOR_AF]
      ? parseFloat(row[ColunaExcel.VALOR_AF]).toFixed(2)
      : '0.00';
    const valorAG = row[ColunaExcel.VALOR_AG]
      ? parseFloat(row[ColunaExcel.VALOR_AG]).toFixed(2)
      : '0.00';
    ...
    const historicoI = DataFormatter.normalizeText(
      row[ColunaExcel.HISTORICO_I]?.toString() || '',
    );
    const histG = DataFormatter.normalizeText(
      row[ColunaExcel.HISTORICO_G]?.toString() || '',
    );
    const historicoAM = DataFormatter.normalizeText(
    ...
    if (contaBancaria === '13') {
      results.push(
        `${localImportacao};${dataFormatada};${contaDebito};13;${valorO};${historicoCompleto}`,
      );
      if (valorAC !== '0.00') {
        results.push(
          `${localImportacao};${dataFormatada};13;1111;${valorAC};0;Desconto s/ Duplic. N: ${historicoI} - ${histG}`,
        );
      }
      if (valorAG !== '0.00') {
        results.push(
          `${localImportacao};${dataFormatada};13;1111;${valorAG};0;Taxa Administr. N: ${historicoI} - ${histG}`,
        );
      }
      if (valorAD !== '0.00' || valorAF !== '0.00') {
        const totalJuros = (parseFloat(valorAD) + parseFloat(valorAF)).toFixed(
          2,
        );
        results.push(
          `${localImportacao};${dataFormatada};1381;13;${totalJuros};0;Pagto Juros s/ Duplic. N: ${historicoI} - ${histG}`,
        );
      }
      if (valorAE !== '0.00') {
        results.push(
          `${localImportacao};${dataFormatada};1383;13;${valorAE};0;Pagto Multa s/ Duplic. N: ${historicoI} - ${histG}`,
        );
      }
    } else {
      if (importLocation === TipoEmpresa.MATRIZ) {
        results.push(
          `${localImportacao};${dataFormatada};${contaDebito};${contaBancaria};${valorO};${historicoCompleto}`,
        );
      } else if (importLocation === TipoEmpresa.FILIAL) {
        results.push(
          `${localImportacao};${dataFormatada};${contaDebito};1516;${valorO};${historicoCompleto}`,
        );
        results.push(
          `0001;${dataFormatada};${MAPA_EXTRA_DISCOUNT_MAP.get(importLocation) || '1513'};${contaBancaria};${valorO};${historicoCompleto}`,
        );
      }
      ...
      if (valorAC !== '0.00') {
        if (importLocation === TipoEmpresa.MATRIZ) {
          results.push(
            `${localImportacao};${dataFormatada};1111;${contaBancaria};${valorAC};0;Desconto s/ Duplic. N: ${historicoI} - ${histG}`,
          );
        } else if (importLocation === TipoEmpresa.FILIAL) {
          results.push(
            `${localImportacao};${dataFormatada};1514;1111;${valorAC};0;Desconto s/ Duplic. N: ${historicoI} - ${histG}`,
          );
          results.push(
            `0001;${dataFormatada};${MAPA_EXTRA_DISCOUNT_MAP.get(importLocation)};1111;${valorAC};0;Desconto s/ Duplic. N: ${historicoI} - ${histG}`,
          );
        }
      }
      ...
      if (valorAG !== '0.00') {
        if (importLocation === TipoEmpresa.MATRIZ) {
          results.push(
            `${localImportacao};${dataFormatada};1111;${contaBancaria};${valorAG};0;Taxa Administr. N: ${historicoI} - ${histG}`,
          );
        } else if (importLocation === TipoEmpresa.FILIAL) {
          results.push(
            `${localImportacao};${dataFormatada};1514;1111;${valorAG};0;Taxa Administr. N: ${historicoI} - ${histG}`,
          );
          results.push(
            `0001;${dataFormatada};${MAPA_EXTRA_DISCOUNT_MAP.get(importLocation)};1111;${valorAG};0;Taxa Administr. N: ${historicoI} - ${histG}`,
          );
        }
      }
      ...
      if (valorAD !== '0.00' || valorAF !== '0.00') {
        ...
        if (importLocation === TipoEmpresa.MATRIZ) {
          results.push(
            `${localImportacao};${dataFormatada};1381;${contaBancaria};${totalJuros};0;Pagto Juros s/ Duplic. N: ${historicoI} - ${histG}`,
          );
        } else if (importLocation === TipoEmpresa.FILIAL) {
          results.push(
            `${localImportacao};${dataFormatada};1381;1516;${totalJuros};0;Pagto Juros s/ Duplic. N: ${historicoI} - ${histG}`,
          );
          results.push(
            `0001;${dataFormatada};${MAPA_EXTRA_JUROS_MULTA_MAP.get(importLocation)};${contaBancaria};${totalJuros};0 - Pagto Juros s/ Duplic. N: ${historicoI} - ${histG}`,
          );
        }
      }
      ...
      if (valorAE !== '0.00') {
        if (importLocation === TipoEmpresa.MATRIZ) {
          results.push(
            `${localImportacao};${dataFormatada};1383;${contaBancaria};${valorAE};0;Pagto Multa s/ Duplic. N: ${historicoI} - ${histG}`,
          );
        } else if (importLocation === TipoEmpresa.FILIAL) {
          results.push(
            `${localImportacao};${dataFormatada};1383;1516;${valorAE};0;Pagto Multa s/ Duplic. N: ${historicoI} - ${histG}`,
          );
          results.push(
            `0001;${dataFormatada};${MAPA_EXTRA_JUROS_MULTA_MAP.get(importLocation)};${contaBancaria};${valorAE};0;Pagto Multa s/ Duplic. N: ${historicoI} - ${histG}`,
          );
        }
      }
    }
  ...
  static async processarFiscal326(
    ...
    for (const row of rows) {
      ...
      const tipoEmpresaFiscal =
        importLocation === TipoEmpresa.MATRIZ ? '1' : '2';
