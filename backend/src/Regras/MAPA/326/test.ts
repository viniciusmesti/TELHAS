...
async function main() {
  const baseDir = 'C:\\Users\\Administrador\\Desktop\\DEPLOY\\backend\\src\\uploads\\MAPA';
  ...
  const outputContabilPath = path.join(baseDir, 'saida', 'contabil', 'contabil326_mapa.txt');
  const outputFiscalPath = path.join(baseDir, 'saida', 'fiscal', 'fiscal326_mapa.txt');
  const outputDuplicatasPath = path.join(baseDir, 'saida', 'duplicatas', 'duplicatas_nao_encontradas326_mapa.xlsx');
