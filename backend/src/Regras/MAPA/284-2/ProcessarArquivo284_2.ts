...
export function transformData(rows: any[]): string[] {
  ...
  rows.forEach((row, index) => {
    ...
    if (!['6', '9'].includes(empresa)) {
      console.log(
        `Linha ${index + 1} ignorada: empresa inválida (${empresa}).`,
      );
      return;
    }
    ...
    const banco = row[37]?.toString().trim() || '';
    ...
    if (empresa === '6') {
      // MATRIZ
      output.push(
        `${local};${dataBaixa};${banco};${contaMaquina};${valorBaixa};1189;${histBase}`,
      );
      output.push(
        `${local};${dataBaixa};1325;${contaMaquina};${taxaAdm};1360;${histBase}`,
      );
    } else if (empresa === '9') {
      // FILIAL
      output.push(
        `${local};${dataBaixa};1514;${contaMaquina};${valorBaixa};1189;${histBase}`,
      );
      output.push(
        `${local};${dataBaixa};1325;${contaMaquina};${taxaAdm};1360;${histBase}`,
      );
      ...
      // EXTRA NA MATRIZ
      output.push(
        `0001;${dataBaixa};${banco};1515;${valorBaixa};1189;${histBase}`,
      );
    }
  });
...
export async function processarArquivo284_2(
