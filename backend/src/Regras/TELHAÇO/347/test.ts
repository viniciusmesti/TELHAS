...
async function testarProcessamento() {
  try {
    ...
    const inputExcelPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAÇO', 'uploads', '347.xlsx');
    const outputTxtPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAÇO', 'saida', 'saida347.txt');
    ...
    console.log('Teste da regra 347 concluído com sucesso! Verifique o arquivo:', outputTxtPath);
  } catch (error) {
