...
async function testarProcessamento() {
  try {
    ...
    const inputExcelPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAÇO', 'uploads', '336.xlsx');
    const outputTxtPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAÇO', 'saida', 'saida336.txt');
    ...
    console.log('Teste da regra 336 concluído com sucesso! Verifique o arquivo:', outputTxtPath);
  } catch (error) {
