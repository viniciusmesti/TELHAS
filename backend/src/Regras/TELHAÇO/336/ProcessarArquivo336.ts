...
export async function readExcelFile(filePath: string): Promise<any[]> {
  ...
  console.log("Planilhas disponíveis:", workbook.worksheets.map(s => s.name));
  const rows: any[] = [];
...
export function processarRegra336(rows: any[]): string[] {
  ...
  rows.forEach(row => {
    ...
    const local =
      filial === "1" ? CODE_MATRIZ :
      filial === "2" ? CODE_PARANAVAI :
      filial === "3" ? CODE_UMUARAMA :
      CODE_CAMPO_MOURAO;
    ...
    if (+valorDesdob > 0) {
      output.push(`${local};${dataBaixa};893;${cnpjCliente};${valorDesdob};2081;${historicoBase}`);
    }
    if (+jurosRecebidos > 0) {
