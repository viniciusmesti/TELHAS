import { processarArquivo289Telhas, exportToTxt289 } from './processarArquivo289';
...
async function testarProcessamento() {
  try {
    ...
    // Caminhos de saída
    const outputContabilPath = path.join(baseDir, 'saida', 'contabil', 'contabil289_telhas.txt');
    const outputFiscalPath = path.join(baseDir, 'saida', 'fiscal', 'fiscal289_telhas.txt');
    const outputDuplicatasPath = path.join(baseDir, 'saida', 'duplicatas', 'duplicatas_nao_encontradas289_telhas.xlsx');
