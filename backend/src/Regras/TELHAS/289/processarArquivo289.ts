...
function validarDadosPagamento(dados: DadosPagamento): boolean {
  return dados.relatorio === CONFIG.VALORES.RELATORIO_ESPERADO &&
         [CONFIG.VALORES.TIPO_EMPRESA_MATRIZ, 
          ...
          CONFIG.VALORES.TIPO_EMPRESA_FILIAL_6].includes(dados.importLocation);
}
...
function processarContabil(
  ...
  if (debito === '0000') {
    console.warn(`⚠️ Tipo de despesa não mapeada: "${dados.tipoDespesa}" — lançamento ignorado.`);
    return;
  }
...
function processarFiscal(
  ...
  const tipoEmpresaFiscal = dados.importLocation === CONFIG.VALORES.TIPO_EMPRESA_MATRIZ ? '1' :
                            ...
                            (dados.importLocation === CONFIG.VALORES.TIPO_EMPRESA_FILIAL_4 ? '4' : 
                            (dados.importLocation === CONFIG.VALORES.TIPO_EMPRESA_FILIAL_5 ? '5' : '6')));
