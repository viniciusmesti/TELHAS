...
function processRow(row: ExcelRow, index: number, output: string[]): void {
  ...
  let localCode = '';
  if (filial === '5') localCode = MATRIZ_CODE;
  else if (filial === '7') localCode = FILIAL_CODE_3;
  else if (filial === '10') localCode = FILIAL_CODE_4;
  else if (filial === '11') localCode = FILIAL_CODE_5;
  else if (filial === '12') localCode = FILIAL_CODE_6;
