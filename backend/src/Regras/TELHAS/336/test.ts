...
async function testarProcessamento() {
  try {
    ...
    const inputExcelPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAS', 'uploads', '336.xlsx');
    const outputTxtPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAS', 'saida', 'saida336.txt');
    ...
    console.log('Teste da Regra 336 (TELHAS) concluído com sucesso! Verifique o arquivo:', outputTxtPath);
  } catch (error) {
