...
export function processarRegra328(rows: any[]): string[] {
  ...
  rows.forEach((row, index) => {
    ...
    if (!['5', '7', '10', '11', '12'].includes(filial)) {
      console.log(
        `Linha ${index + 1} ignorada - Empresa/Filial inválida: ${filial}`,
      );
      return;
    }
    ...
    if (banco === 13) {
      ...
      if (filial === '5') {
        ...
        output.push(
          `${local};${dataBaixa};893;999;${valorBaixa};${historicoBase}`,
        );
        ...
        output.push(
          `0001;${dataBaixa};${contaExtra};${banco};${valorBaixa};${historicoBase}`,
        );
      }
    }
  });
