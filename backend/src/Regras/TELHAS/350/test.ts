...
async function testarProcessamento() {
  try {
    ...
    const inputExcelPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAS', 'uploads', '350.xlsx');
    const outputTxtPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAS', 'saida', 'saida350.txt');
    ...
    console.log('Teste da regra 350 para TELHAS concluído com sucesso! Verifique o arquivo:', outputTxtPath);
  } catch (error) {
