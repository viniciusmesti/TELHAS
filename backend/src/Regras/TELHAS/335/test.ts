...
async function testarProcessamento() {
  try {
    ...
    const inputExcelPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAS', 'uploads', '335.xlsx');
    const outputTxtPath = path.join(__dirname, '..', '..', '..', 'uploads', 'TELHAS', 'saida', 'saida335.txt');
    ...
    console.log('Teste da regra 335 para TELHAS concluído com sucesso! Verifique o arquivo de saída:', outputTxtPath);
  } catch (error) {
